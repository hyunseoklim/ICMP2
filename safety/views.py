...
@login_required(login_url="login")
def mysafety_admin_history(request):
    if not request.user.is_staff:
        return redirect("dashboard")

    from facilities.models import Worker
    workers = Worker.objects.filter(status="active").select_related("user")
    ...
    weeks = []
    if worker_id:
        selected_worker = get_object_or_404(Worker, pk=worker_id)
        sessions_qs = SafetyCheckSession.objects.filter(worker=selected_worker)
        weeks = _build_calendar(sessions_qs, year, month)

    prev_month = date(year, month, 1) - timedelta(days=1)
    next_month = (date(year, month, 28) + timedelta(days=4)).replace(day=1)

    return render(request, "safety/mysafety_admin_history.html", {
        "workers": workers,
