"""
gas/modules/arima — 가스 채널 ARIMA 미래값 예측 모듈.
...
표준 사용 예:
    >>> predictor = GasARIMAPredictor()
    >>> cp_anchor = cp_detector.anchor_status("gas_A", "voc")
    >>> result = predictor.predict(history, cp_anchor)
    >>> if result.path == "normal":
    ...     print(result.forecast_mean[-1], result.drift_tstat)
"""
...
@dataclass
class ARIMAResult:
    """가스 ARIMA 모듈의 단일 채널 *원시 예측* 결과.

    원시 예측 + 신뢰성 메타데이터만 담는다. 2축 확신도 등급
    (ForecastConfidence)·K-확인은 통합 레이어(출력 정책)가 본 결과를
    입력받아 산출한다.
    ...
    Attributes:
        ...
        device_id: 장비 식별자.
        sensor_type: 센서 종류.
        path: 예측 경로 — 'normal'(정규 ARIMA 재적합) / 'degraded'
              (contamination 시 robust drift 보조) / 'unknown'(예측 불가).
        forecast_mean: H시점 점예측 리스트. path='unknown'이면 빈 리스트.
        ci_lower: H시점 95% 신뢰구간 하한 (raw). 'unknown'이면 빈 리스트.
        ci_upper: H시점 95% 신뢰구간 상한 (raw). 'unknown'이면 빈 리스트.
        forecast_steps: 예측 수평 H.
        drift: 적합된 drift(추세) 계수. normal=statsmodels 추세계수,
               degraded=median 차분, unknown=None.
        drift_tstat: |drift| / SE(drift) — drift 통계 유의성.
                     normal 경로만 산출. degraded·unknown은 None.
        segment_length: 재적합에 사용된 CP-anchored 구간 길이.
        anchor_index: 사용된 CP anchor 인덱스 (참고용, None 가능).
        order: ARIMA 차수.
        reason: 판정 사유 한글 문자열.
    """
...
# ============================================================================
# GasARIMAPredictor — 본체 클래스
# ============================================================================

class GasARIMAPredictor:
    """가스 채널 ARIMA 예측기 — predict 시점 CP-anchored 재적합.

    학습 상태가 없다(stateless). 설정만 보관하고, predict() 호출마다
    주어진 history의 CP-anchored 구간에 ARIMA를 재적합하여 예측한다.

    인터페이스:
        - __init__: 설정(차수·수평·구간 한계 등).
        - predict: (history, CPAnchorResult) → ARIMAResult.
    """
    ...
    def predict(self, history, cp_anchor: CPAnchorResult) -> ARIMAResult:
        ...
        # 1. 구간 길이 결정 — anchor 있으면 anchor 이후, 없으면 가용 전체 (상한 적용)
