...
from devkit.core.enums import RiskLevel, WorkMode
from devkit.core.modules import ThresholdClassifier
# 수직 분리: 도메인 detector는 자기 core enum을 반환하므로 비교도 도메인 enum으로
from gas.core.enums import RiskLevel as GasRiskLevel
from power.core.enums import RiskLevel as PowerRiskLevel
...
from devkit.generator.gas_generator import (
    generate_gas_normal_pool, split_pool_by_device,
    generate_scenario as gen_gas_scenario,
)
from devkit.generator.power_generator import (
...
class TestEndToEndGas:
    ...
    def test_full_gas_workflow(self):
        ...
        caution_count = sum(1 for r in late_results if r.level == GasRiskLevel.CAUTION)
...
class TestEndToEndPower:
    ...
    def test_full_power_workflow(self):
        ...
        late_results = [detector.predict(b) for b in scenario[150:]]
        caution_count = sum(1 for r in late_results if r.level == PowerRiskLevel.CAUTION)
        # 위험 구간 50개 중 대부분 CAUTION
        assert caution_count >= 40
...
class TestB1Corroboration:
    ...
    def test_grade_unchanged_by_present_level(self):
        ...
        from devkit.core.integration.forecast_policy import ForecastPolicy
    ...
    def test_corroborated_when_present_and_forecast_both_risk(self):
        ...
        from devkit.core.integration.forecast_policy import ForecastPolicy
        from devkit.core.enums import ForecastConfidence
        ar = self._arima_result(mean=100.0, ci_lower=30.0, ci_upper=175.0)
