 ...
 * 참조: API_BASE, cellWidth, cellHeight, imgWidth, imgHeight,
 *       currentFloorId, imageOverlay, map (map_config.js)
 *       drawGrid() (map_grid.js), loadZones() (map_zone.js)
 * 출력:
 *   - currentFloorId 갱신
 *   - API에서 격자 설정 수신 후 cellWidth/cellHeight/imgWidth/imgHeight 갱신
 *   - imageOverlay bounds 갱신 + 지도 뷰 재조정
 *   - drawGrid() 재호출로 격자 갱신
 *   - loadZones(), loadGeofences(), loadSensors(), startWorkerSim() 연쇄 호출
 *
 * [수정 내역]
 *   - drawGrid 호출 인자 오타 수정: cellHeightight → cellHeight
 */
function loadFloorData(floorId) {
  ...
  fetch(`${API_BASE}/floor-grids/?floor_id=${floorId}`)
    ...
    .then(data => {
      if (data.length > 0) {
        const g   = data[0];
        cellWidth  = g.cell_width;
        cellHeight = g.cell_height;
        imgWidth   = g.img_width;
        imgHeight  = g.img_height;
